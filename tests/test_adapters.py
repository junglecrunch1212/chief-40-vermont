...
async def test_bluebubbles_send():
    ...
    with patch.dict(os.environ, {"BLUEBUBBLES_URL": "http://localhost:1234", "BLUEBUBBLES_SECRET": "test-secret"}):
...
async def test_bluebubbles_ping_failure():
    ...
    with patch.dict(os.environ, {"BLUEBUBBLES_URL": "http://localhost:1234", "BLUEBUBBLES_SECRET": "secret"}):
        sender = BlueBubblesSender()
        sender._client = AsyncMock()
