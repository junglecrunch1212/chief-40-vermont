...
class TestConsolePrivacyFence:
    ...
    async def test_james_cannot_see_laura_privileged_sensors(self, db):
        ...
        # Insert Laura's privileged sensor reading (id is AUTOINCREMENT, omit it)
        await db.execute(
            """INSERT INTO pib_sensor_readings
               ...
               VALUES (?, ?, ?, ?, ?, ?, 'high', 60, '2026-03-05T06:00:00Z', ?)""",
            ["health", "sleep", "m-laura", "2026-03-04T06:00:00", '{"hours":7}', "privileged", "idem-priv-laura-001"],
        )
        ...
        await db.execute(
            """INSERT INTO pib_sensor_readings
               (sensor_id, reading_type, member_id, timestamp, value,
                classification, confidence, ttl_minutes, expires_at, idempotency_key)
               VALUES (?, ?, ?, ?, ?, ?, 'high', 60, '2026-03-05T06:00:00Z', ?)""",
            ["health", "sleep", "m-james", "2026-03-04T06:00:00", '{"hours":6}', "normal", "idem-priv-james-001"],
        )
        await db.commit()
        ...
        # Cleanup
        await db.execute("DELETE FROM pib_sensor_readings WHERE idempotency_key IN ('idem-priv-laura-001', 'idem-priv-james-001')")
        await db.commit()
    ...
    async def test_laura_sees_own_privileged_sensors(self, db):
        ...
        await db.execute(
            """INSERT INTO pib_sensor_readings
               (sensor_id, reading_type, member_id, timestamp, value,
                classification, confidence, ttl_minutes, expires_at, idempotency_key)
               VALUES (?, ?, ?, ?, ?, ?, 'high', 60, '2026-03-05T06:00:00Z', ?)""",
            ["health", "sleep", "m-laura", "2026-03-04T06:00:00", '{"hours":7}', "privileged", "idem-priv-laura-003"],
        )
        await db.commit()
        ...
        rows = await db.execute_fetchall(
            """SELECT * FROM pib_sensor_readings
               WHERE (classification != 'privileged' OR member_id = ?)
               AND idempotency_key = 'idem-priv-laura-003'""",
            ["m-laura"]
        )
        ...
        # Cleanup
        await db.execute("DELETE FROM pib_sensor_readings WHERE idempotency_key = 'idem-priv-laura-003'")
        await db.commit()
